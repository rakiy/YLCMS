...
class AttachmentAction extends  Action {
		...
		if(!$this->userid){
			if($this->isadmin){
				...
				if(cookie('auth')){
					...
					$Ycms_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$HTTP_USER_AGENT);
					list($userid, $groupid ,$password) = explode("-", authcode(cookie('auth'), 'DECODE', $Ycms_auth_key));
					$this->userid = $userid;
	...
	public function index(){
		...
		$postd=array('isadmin','more','isthumb','file_limit','file_types','file_size','moduleid','savePath');
		...
		$enupsetup = authcode($auth, 'DECODE', $Ycms_auth_key);
		...
		$this->assign('savePath',$_REQUEST['savePath'),
