...
class IndexAction extends AdminbaseAction
{
	...
	public function main() {
		...
		$info = array(
			...
			'GD'=> function_exists("imagecreate")?'支持':'不支持',
            );
		$Ycms_info=array(
			'Ycms_VERSION'=> C('Pro_ver') ,			
			'Core'=> 'ThinkPHP 3.1.3',
			'SN'=> $sites['value'] ,
			'update'=>  date('Y年m月d日',C('Pro_update')),
			'edittime'=> '2013年05月30日',
			'author' => 'Rakiy [xux851@gmail.com]'
			
		);
		$this->assign('Ycms_info',$Ycms_info);
        $this->assign('server_info',$info);	
		foreach ((array)$this->module as $rw){
			if($rw['type']==1 && $rw['id'] < 4){  
				$molule= M($rw['name']);
		...
		$tip = ($userinfos['username'] == 'admin')?1:0;
