...
class AdminbaseAction extends Action
{
	...
	public function create_show($id,$module)
    {
		...
		if(APP_LANG){
			...
			$T = F('config_'.LANG_NAME,'', './Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}else{
			L(include LANG_PATH.$this->sysConfig['DEFAULT_LANG'].'/common.php');
			$T = F('config_'.$this->sysConfig['DEFAULT_LANG'],'', './Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
		}
		$this->assign('T',$T);
		...
		if($CONTENT_POS !== false){
				...
				foreach ($pageurls as $p=>$urls) {
					...
					$this->buildHtml($filename,$dir,'./Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
				}
		}else{
		...
		if($cat['moduleid']==1){
			...
			if($CONTENT_POS !== false){
					...
					foreach ($pageurls as $p=>$urls) {
						...
						$r=$this->buildHtml($filename,$dir,'./Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
					}
			}else{
					$this->assign($data);
			...
			$r=$this->buildHtml($filename,$dir,'./Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
	...
	public function create_index($sitemap)
    {
		...
		if(empty($sitemap)){
			...
			$this->assign('bcid',0);
			$r=$this->buildHtml('index','./'.$lang,'./Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
		}else{
			$this->assign('sitemap','1');
			$r=$this->buildHtml('sitemap','./'.$lang,'./Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
		}
		if($r) return true;
    }
