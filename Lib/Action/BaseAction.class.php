...
class BaseAction extends Action{
    ...
    public function _initialize() {
		...
		if(APP_LANG){
			...
			if(I('get.l','','get_safe_replace')){
				if(!$this->Lang[I('get.l')]['status'])$this->error (L('NO_LANG'));
				$lang=$_GET['l'];
			}else{
		...
		$session = new Online();
		if(cookie('auth')){
			$Ycms_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$_SERVER['HTTP_USER_AGENT']);
    ...
    public function index($catid='',$module='')
    {
		...
		if($module=='Guestbook'){
			...
			$modle1 = M('category');
			$data1 = $modle1->find($catid);
			if($data1['listtype'] != '0'){
				$arr = explode(',', $data1['arrchildid']);
				if (isset($arr[1])){
					$data = $modle->find($arr[1]);
				}	
			}
			unset($data['id']);
