...
class Page extends Think {
	...
	public function  show(){
		...
		$output = '';
		$output .= '<a class="a1">'.L('共').$this->totalRows.L('条').'</a>';
		$output .= '<a href="'.$this->pageurl($urlrule, 1,$this->parameter).'">'.L('首页').'</a>';
		$output .= '<a href="'.$this->pageurl($urlrule, $pre_page,$this->parameter).'">'.L('上一页').'</a>';
		...
		$output .='<a href="'.$this->pageurl($urlrule,$next_page,$this->parameter).'">'.L('下一页')."</a>"; 
		$output .='<a href="'.$this->pageurl($urlrule,$this->totalPages,$this->parameter).'">'.L('末页')."</a>";
		return $output;
	}
