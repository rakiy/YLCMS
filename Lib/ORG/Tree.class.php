...
class Tree extends Think {
	...
	function get_nav($bid,$maxlevel,$effected_id='navlist',$style='filetree ' ,$homefont='',$recursion=FALSE ,$child='',$enhomefont='',$lang='') {
        ...
        foreach($child as $id=>$a) {
			...
			$floder_status .=  $number==$total ?  ' class="navfoot '.$foldertype.'"' :  ' class="'.$first.$foldertype.'"';
            ...
            if($ischild && $level < $this->level){
			   ...
			   $this->ret .= '<span class="fl_ico"></span><a href="'.$url.'" title="'.$catname.'" id="nav_a_'.$id.'"><span class="fl">'.$catname.'</span>'.$enzm.'</a>';
            }
